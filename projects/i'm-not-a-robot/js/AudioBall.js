...
Displays the AudioBall class and it's subsequent functions
***********************/

class AudioBall { // Creates a class that is called from script.js
  constructor(x, y) {
    this.x = x; // Defines the x postion in script.js.
    this.y = y; // Defines the x postion in script.js.
    this.size = 100; // Assigns a ball size.
    this.vx = 0; // Assigns an intial horiozontal velocity to 0.
    this.vy = 0; // Assigns an intial verticall velocity to 0.
    this.gravity = 0.2; // Assigns a downward gravity value of 0.2.
    this.fill = 255; // Defines the fill colour to white.
    this.alive = true; // Starts the audioBall as visible.
  }

  checkHit(obstacle) {
    // Checks whether the audioCar's centre-point overlaps a obstacle
    if (
  ...
  handleInput() {
    // Uses microphone input to translate audio car up the y axis.
    ...
    let step = map(micLevel, 0, 0.5, 0, 10); // Create variable that maps the car location to the microphone input
    ...
    if (keyIsDown(UP_ARROW)) {
      // Hold "up" arrow key to move the ball upwards.
      this.vy = -1; // Decreases y velocity to move upwards
    }

    if (keyIsDown(RIGHT_ARROW)) {
      // Hold "right" arrow key to move the ball to the right
      this.vx = 1; // Increases x velocity to move to the right
    } else if (keyIsDown(LEFT_ARROW)) {
      // Hold "left" arrow key to move the ball to the left
      this.vx = -1; // Decreases x velocity to move to the left
    }
  }

  move() {
    // Add velocity to position for movement
  ...
  display() {
    // Displays the audioCar as a circle
