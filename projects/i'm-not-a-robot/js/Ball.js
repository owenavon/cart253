...
Displays the Balll class and it's subsequent functions
***********************/
...
class Ball { // Creates a class that is called from script.js
  constructor(x, y, clickSFX) {   // function that creates an instance of a class.
    ...
    this.ballColor = {
      // Assigns an RGB value to the ball.
      r: 100,
      g: 200,
      b: 300,
    };
  ...
  gravity(force) {
    // Function which is called in updateBall();
  ...
  bounce(paddle) {
    ...
    if (
      ...
      // States that the ball is within the height of the paddle.
  ...
  tryToTouchToken(token) {
    // Function which is called in updateToken();
    ...
    if (d < this.size / 2 + token.size / 2) {
      // Indiactes where the ball and token will overlap.
